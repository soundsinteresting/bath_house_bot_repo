#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
from Utils import log, cards_order
from Utils import ORDER_DICT2, INIT_CARDS, SCORE_DICT
...
class MrGreed(MrRandom):
    ...
    def gen_scenario(void_info, cards_remain, lens):
        ...
        while True:
            ...
            if MrGreed.check_void_legal(cards_remain[0:lens[0]], cards_remain[lens[0]:lens[0] + lens[1]],
                                        cards_remain[lens[0] + lens[1]:], void_info):
                break
        cards_list_list = [cards_remain[0:lens[0]], cards_remain[lens[0]:lens[0] + lens[1]],
                           cards_remain[lens[0] + lens[1]:sum(lens)]]
        a2b = [];
        a2c = []
    ...
    def pick_a_card(self):
        assert (self.cards_on_table[0] + len(
            self.cards_on_table) - 1) % 4 == self.place, "self.place and self.cards_on_table contrdict"
        global print_level
            ...
            for ax in range(MrGreed.N_SAMPLE):
                ...
                for c in d_legal:
                    ...
                    MrGreed.as_third_player(suit, four_cards, cards_dict_1, cards_list_1, cards_dict_2, cards_list_2
                                            , fmt_score_list, self.cards_on_table[0], scs_rmn_avg, impc_dict,
                                            self.place)
        ...
        # 如果我是第一个出
            ...
            for ax in range(MrGreed.N_SAMPLE):
                ...
                for c in d_legal:
                    four_cards[0] = c
                    MrGreed.as_second_player(c[0], four_cards, cards_dict_1, cards_list_1, cards_dict_2, cards_list_2,
                                             cards_dict_3, cards_list_3
                                             , fmt_score_list, self.cards_on_table[0], scs_rmn_avg, impc_dict,
                                             self.place)
                    score = -1 * MrGreed.clear_score(four_cards, fmt_score_list, self.cards_on_table[0], scs_rmn_avg) \
                            + MrGreed.calc_relief(c, impc_dict, scs_rmn_avg, fmt_score_list[self.place][0])
...
def test_1st():
    ...
    g0.history = [(0, 'C2', 'C3', 'C4', 'C5'), (3, 'C6', 'C7', 'C8', 'D2'), (2, 'D3', 'D4', 'D5', 'D6'),
                  (1, 'S2', 'S3', 'S4', 'S5')]
